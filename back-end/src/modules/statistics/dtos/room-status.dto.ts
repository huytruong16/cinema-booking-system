...
export class RoomStatusDto {
    ...
    @ApiProperty({ description: 'Trạng thái phòng: Đang chiếu, Trống, Sắp chiếu', enum: ScreeningRoomStatusEnum })
    ...
    @ApiProperty({ description: 'Suất chiếu đang diễn ra', type: ShowtimeInfo, required: false })
    SuatChieuHienTai?: ShowtimeInfo;

    @ApiProperty({ description: 'Suất chiếu sắp tới', type: UpcomingShowtimeInfo, required: false })
