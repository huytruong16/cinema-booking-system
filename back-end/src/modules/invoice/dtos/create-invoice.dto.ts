...
export class CreateInvoiceDto {
    ...
    @IsUUID("4", { each: true, message: 'Mã ghế suất chiếu phải là UUID v4 hợp lệ' })
    MaGheSuatChieus: string[];

    @ApiProperty({ description: 'Danh sách mã combo (nếu có)', example: [{ MaCombo: 'c1d2e3f4-a5b6-7890-cdef-ab1234567890', SoLuong: 1 }] })
    @IsArray({ message: 'Danh sách mã combo phải là một mảng' })
    @IsOptional()
    Combos?: ComboDto[];

    @ApiProperty({ description: 'Danh sách mã giảm giá người dùng (nếu có)', example: ['c1a2u3c4h5e6r7-890ab-cdef-1234567890ab'] })
