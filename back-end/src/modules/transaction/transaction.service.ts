...
export class TransactionService {
      ...
      Showtime: {
        ...
        PhienBanPhim: {
          Phim: {
            ...
            DinhDang: {
              TenDinhDang:
                invoice.Ves[0].GheSuatChieu.SuatChieu.PhienBanPhim.DinhDang
                  .TenDinhDang,
            },
            NgonNgu: {
              TenNgonNgu:
                invoice.Ves[0].GheSuatChieu.SuatChieu.PhienBanPhim.NgonNgu
                  ?.TenNgonNgu,
            },
          },
        },
        PhongChieu: {
          TenPhongChieu:
            invoice.Ves[0].GheSuatChieu.SuatChieu.PhongChieu.TenPhongChieu,
        },
      },
    };
  ...
  async updateTransactionStatus(webhookBody: any) {
    const { data, verified } =
      this.payosService.verifyPaymentWebhookData(webhookBody);
    ...
    const isPaid =
      code === '00' || data?.status === 'PAID' || data?.success === true;

    await this.prisma.gIAODICH.update({
      where: { MaGiaoDich: transaction.MaGiaoDich },
      data: {
        TrangThai: isPaid
          ? TransactionStatusEnum.THANHCONG
          : TransactionStatusEnum.THATBAI,
        UpdatedAt: new Date(),
      },
    });
  ...
  async updateTransactionMethod(
    ...
    if (
      transaction.LoaiGiaoDich === TransactionTypeEnum.MUAVE &&
      transaction.PhuongThuc == TransactionEnum.TRUCTIEP &&
      transactionMethod == TransactionEnum.TRUCTUYEN &&
      transaction.GiaoDichUrl.length === 0
    ) {
      paymentData = await this.payosService.getPaymentLinkUrl(
        Number(transaction.Code),
        Number(transaction.HoaDon.TongTien),
        `${transaction.HoaDon.Code}`,
      );
    }
  ...
  async createRefundTransaction(payload: CreateRefundTransactionDto) {
    ...
    if (
      ...
      this.request?.user?.vaitro !== RoleEnum.ADMIN
    ) {
    ...
    if (!refundRequests) {
      throw new BadRequestException(
        'Yêu cầu hoàn vé không tồn tại hoặc đã được xử lý',
      );
    }
  ...
  async updateRefundTransactionStatus(
    ...
    if (transaction.LoaiGiaoDich !== TransactionTypeEnum.HOANTIEN) {
      throw new BadRequestException(
        'Giao dịch không phải là giao dịch hoàn tiền, không thể cập nhật trạng thái hoàn tiền',
      );
    }
    ...
    if (newStatus === TransactionStatusEnum.THANHCONG) {
      for (const refundRequestId of transaction.YeuCauHoanVes.map(
        (req) => req.MaYeuCau,
      )) {
        await this.refundRequestService.updateRefundRequestStatus(
          refundRequestId,
          { TrangThai: RefundRequestStatusEnum.DAHOAN },
          TransactionEnum.TRUCTUYEN,
        );
      }
    }
    if (newStatus === TransactionStatusEnum.THATBAI) {
      for (const refundRequestId of transaction.YeuCauHoanVes.map(
        ...
        await this.refundRequestService.updateRefundRequestStatus(
          refundRequestId,
          { TrangThai: RefundRequestStatusEnum.DAHUY },
          TransactionEnum.TRUCTUYEN,
        );
      }
    }
