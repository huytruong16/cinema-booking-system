...
export class TransactionCronService {
  ...
  async handleCron() {
    ...
    const expiredTransactions =
      await this.findExpiredTransactions(tenMinutesAgo);
  ...
  private async rollbackTicket(invoice: any) {
    const ticketIds = (invoice.Ves || [])
      .map((v: any) => v.MaVe)
      .filter(Boolean);
    const seatIds = (invoice.Ves || [])
      .map((v: any) => v.GheSuatChieu?.MaGheSuatChieu)
      .filter(Boolean);
    const promoIds = (invoice.HoaDonKhuyenMais || [])
      .map((p: any) => p.MaKhuyenMaiKH)
      .filter(Boolean);
