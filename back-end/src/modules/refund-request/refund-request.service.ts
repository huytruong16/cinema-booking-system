...
export class RefundRequestService {
  ...
  async createNewRefundRequest(
    ...
    const param = await prisma.tHAMSO.findUnique({
      where: { TenThamSo: 'RefundWindowHours' },
    });
    ...
    const refundWindow = new Date(
      inv?.Ves[0]?.GheSuatChieu.SuatChieu.ThoiGianBatDau || new Date(),
    );
    refundWindow.setHours(refundWindow.getHours() + refundWindowHours);
    if (new Date() > refundWindow) {
      throw new BadRequestException(
        'Vé đã vượt quá thời gian hoàn vé cho phép',
      );
    }
    ...
    function validateRefundRequest() {
      const usedTickets = inv!.Ves.filter(
        (ve) => ve.TrangThaiVe === TicketStatusEnum.DASUDUNG,
      );
      if (usedTickets.length > 0) {
        throw new BadRequestException(
          `Vé ${usedTickets.map((ve) => ve.Code).join(', ')} đã được sử dụng, không thể hoàn vé`,
        );
      }

      const refundedTickets = inv!.Ves.filter(
        (ve) => ve.TrangThaiVe == TicketStatusEnum.DAHOAN,
      );
      if (refundedTickets.length > 0) {
        throw new BadRequestException(
          `Vé ${refundedTickets.map((ve) => ve.Code).join(', ')} đã được hoàn, không thể hoàn vé lại`,
        );
      }

      const pendingRefundTickets = inv!.Ves.filter(
        (ve) =>
          ve.TrangThaiVe == TicketStatusEnum.CHOHOANTIEN ||
          ve.TrangThaiVe == TicketStatusEnum.CHUAHOANTIEN,
      );
      if (pendingRefundTickets.length > 0) {
        throw new BadRequestException(
          `Vé ${pendingRefundTickets.map((ve) => ve.Code).join(', ')} đang trong quá trình hoàn tiền, không thể hoàn vé lại`,
        );
      }

      const expiredTickets = inv!.Ves.filter(
        (ve) => ve.TrangThaiVe == TicketStatusEnum.DAHETHAN,
      );
      if (expiredTickets.length > 0) {
        throw new BadRequestException(
          `Vé ${expiredTickets.map((ve) => ve.Code).join(', ')} đã hết hạn hoàn vé`,
        );
      }
    }
  }
  ...
  async updateRefundRequestStatus(
    id: string,
    payload: UpdateRefundRequestStatusDto,
    transactionMethod?: TransactionEnum,
  ): Promise<any> {
    const userRole = this.request?.user?.vaitro;
    const userId = this.request?.user?.id;
    ...
    if (
      userRole === RoleEnum.KHACHHANG &&
      userId !== refundRequest?.HoaDon.KhachHang?.NguoiDungPhanMem?.MaNguoiDung
    ) {
      throw new BadRequestException(
        'Bạn không có quyền cập nhật trạng thái yêu cầu hoàn vé này',
      );
    }

    if (refundRequest.TrangThai === RefundRequestStatusEnum.DAHOAN) {
      throw new BadRequestException(
        `Hóa đơn đã được hoàn tiền, không thể cập nhật trạng thái`,
      );
    }
    ...
    function sendRefundEmailNotification() {
      if (
        (newStatus === RefundRequestStatusEnum.DAHOAN ||
          newStatus === RefundRequestStatusEnum.DAHUY) &&
        ...
        const showTime = new Date(
          firstTicket.GheSuatChieu.SuatChieu.ThoiGianBatDau,
        );
        ...
        const mailData = {
          ...
          MovieName:
            firstTicket.GheSuatChieu.SuatChieu.PhienBanPhim.Phim.TenHienThi,
          CinemaRoom:
            firstTicket.GheSuatChieu.SuatChieu.PhongChieu.TenPhongChieu,
    ...
    async function handleRefundRequestStatus() {
      switch (newStatus) {
        case RefundRequestStatusEnum.DAHOAN:
          if (userRole === RoleEnum.KHACHHANG) {
            throw new BadRequestException(
              'Khách hàng không có quyền cập nhật trạng thái này',
            );
          }
          if (refundRequest!.TrangThai === RefundRequestStatusEnum.DAHUY) {
            throw new BadRequestException(
              `Yêu cầu hoàn vé đã bị hủy, không thể hoàn tiền`,
            );
          }
          if (refundRequest!.MaGiaoDich === null) {
            throw new BadRequestException(
              `Yêu cầu hoàn vé chưa có giao dịch hoàn tiền, không thể hoàn tiền`,
            );
          }
          ...
          for (const ve of refundRequest!.HoaDon.Ves) {
            ...
            await prisma.gHE_SUATCHIEU.update({
              where: {
                MaGheSuatChieu: ve.GheSuatChieu.MaGheSuatChieu,
                DeletedAt: null,
              },
              data: {
          ...
          for (const voucherId of refundRequest!.HoaDon.HoaDonKhuyenMais.map(
            (e) => e.KhuyenMaiKH.MaKhuyenMaiKH,
          )) {
        ...
        case RefundRequestStatusEnum.DAHUY:
          if (refundRequest!.TrangThai !== RefundRequestStatusEnum.DANGCHO) {
            throw new BadRequestException(
              `Yêu cầu hoàn vé không ở trạng thái đang chờ, không thể hủy`,
            );
          }
          if (refundRequest!.MaGiaoDich !== null) {
            throw new BadRequestException(
              `Yêu cầu hoàn vé đang được xử lý hoàn tiền, không thể hủy`,
            );
          }
        ...
        case RefundRequestStatusEnum.DANGCHO:
          throw new BadRequestException(
            'Không thể chuyển trạng thái về đang chờ',
          );
      }
    }
  }
