...
export class GetShowtimeByMovieDto {
  @ApiProperty({
    description:
      'Trạng thái suất chiếu (DANGCHIEU, DACHIEU, SAPCHIEU, DAHUY, CHUACHIEU) — phải gửi chuỗi phân tách dấu phẩy (ví dụ "DANGCHIEU, CHUACHIEU"), mặc định CHUACHIEU',
    required: false,
  })
  @IsOptional()
  @Transform(({ value }) => {
    ...
    return valueList.map(
      (v) => ShowtimeStatusEnum[v.trim()],
    ) as ShowtimeStatusEnum[];
  })
