...
export class PdfService {
  ...
  async generateInvoicePdf(invoice: GetInvoiceResponseDto): Promise<Buffer> {
    ...
    return new Promise((resolve) => {
      ...
      doc.text(
        `Suất chiếu: ${new Date(invoice.ThoiGianChieu).toLocaleString('vi-VN')}`,
        300,
        startY + 40,
      );
      ...
      invoice.Ves.forEach((ticket: any) => {
        ...
        doc.text(
          Number(ticket.DonGia).toLocaleString('vi-VN'),
          priceX,
          currentY,
        );
        doc.text(
      ...
      invoice.Combos.forEach((combo: any) => {
        ...
        doc.text(
          Number(combo.DonGia).toLocaleString('vi-VN'),
          priceX,
          currentY,
        );
        doc.text(
          (Number(combo.DonGia) * combo.SoLuong).toLocaleString('vi-VN'),
          totalX,
          currentY,
        );
        currentY += 20;
  ...
  private drawTicket(doc: PDFKit.PDFDocument, ticket: any) {
    ...
    doc
      .fontSize(10)
      .font('Arial-Bold')
      .text('MOVIX CINEMA', 20, 120, { width: 100, align: 'center' });
    doc
      .fontSize(8)
      .font('Arial')
      .text('123 Đường ABC, TP.HCM', 20, 140, { width: 100, align: 'center' });
    ...
    const mvName =
      movie.TenHienThi.toUpperCase() +
      ' (' +
      showtime.PhienBanPhim.NgonNgu?.TenNgonNgu +
      ', ' +
      showtime.PhienBanPhim.DinhDang?.TenDinhDang +
      ')';
    ...
    doc
      ...
      .fontSize(12)
      .text(
        showtime.ThoiGianBatDau.toLocaleDateString('vi-VN'),
        col1X,
        row1Y + 15,
      );
    ...
    doc
      ...
      .fontSize(12)
      .text(
        showtime.ThoiGianBatDau.toLocaleTimeString('vi-VN', {
          hour: '2-digit',
          minute: '2-digit',
        }),
        col2X,
    ...
    doc
      .font('Arial')
      .fontSize(12)
      .text(room.TenPhongChieu, col1X, row2Y + 15);
    ...
    doc
      .fontSize(16)
      .text(`${Number(ticket.GiaVe).toLocaleString('vi-VN')} đ`, 470, 50);
