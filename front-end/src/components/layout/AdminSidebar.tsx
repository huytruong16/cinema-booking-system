...
// Nhóm 2: QUẢN LÝ NỘI DUNG (Chỉ ADMIN)
...
// Nhóm 3: QUẢN LÝ KINH DOANH & HỆ THỐNG (Chỉ ADMIN)
...
const NavItemLink = ({ item, active, open, onClick }: {
  item: NavItem,
  active: string,
  open: boolean,
...
export default function AdminSidebar({
  ...
  const visibleContentManagementNavItems = useMemo(() => {
    // Chỉ ADMIN
    if (role === "ADMIN") {
      return contentManagementNavItems;
    }
    return []; 
  }, [role]);
  ...
  const visibleSystemManagementNavItems = useMemo(() => {
    // Chỉ ADMIN
    if (role === "ADMIN") {
      return systemManagementNavItems;
    }
    return []; 
  }, [role]);
  ...
  }, [pathname, visibleOperationalNavItems, visibleContentManagementNavItems, visibleSystemManagementNavItems]); 
    ...
    >
      ...
      <nav className="mt-6 flex-1 overflow-y-auto no-scrollbar">
        <ul className="flex flex-col gap-1">
          ...
          {showLabels && (
            <li className="px-2 py-2 text-xs font-semibold text-slate-400 uppercase">
              Nghiệp vụ
            </li>
          )}
