...
export default function ComboManagementPage() {
  ...
  const filteredCombos = useMemo(() => {
    ...
    return combos.filter((combo) => {
      const matchesSearch = combo.TenCombo.toLowerCase().includes(
        searchTerm.toLowerCase()
      );
      const matchesStatus =
        statusFilter === "all" || combo.TrangThai === statusFilter;
    ...
    <Card className="bg-[#1C1C1C] border-slate-800 shadow-lg flex flex-col">
      ...
      <CardContent className="flex-1 space-y-3">
        <div className="flex justify-between items-start gap-2">
          <CardTitle className="text-lg font-semibold text-slate-100 leading-snug line-clamp-2">
            {combo.TenCombo}
          </CardTitle>
          <Badge
            variant="outline"
            className={cn(
              ...
              getBadgeVariant(combo.TrangThai)
            )}
          >
              ...
              <div className="space-y-2">
                ...
                <div className="flex gap-4 items-start">
                  <div className="flex-1">
                    <label
                      htmlFor="file-upload"
                      className="flex flex-col items-center justify-center w-full h-32 border-2 border-dashed border-slate-700 rounded-lg cursor-pointer hover:bg-slate-800/50 transition-colors"
                    >
