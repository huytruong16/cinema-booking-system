...
export default function PromotionManagementPage() {
  ...
  const [editingPromotion, setEditingPromotion] = useState<Voucher | null>(
    null
  );
  ...
  const filteredPromotions = useMemo(() => {
    return promotions.filter((promo) => {
      ...
      const matchesStatus =
        statusFilter === "all" || promo.TrangThai === statusFilter;
...
function PromotionCard({
  ...
  const conditions = useMemo(() => {
    ...
    if (promotion.GiaTriDonToiThieu && promotion.GiaTriDonToiThieu > 0) {
      parts.push(
        `Đơn tối thiểu ${Number(promotion.GiaTriDonToiThieu).toLocaleString(
          "vi-VN"
        )} ₫`
      );
    }
    if (promotion.LoaiGiamGia === "PHANTRAM" && promotion.GiaTriGiamToiDa) {
      parts.push(
        `Giảm tối đa ${Number(promotion.GiaTriGiamToiDa).toLocaleString(
          "vi-VN"
        )} ₫`
      );
    }
      ...
      <CardHeader className="pb-3">
        <div className="flex justify-between items-start gap-2">
          <div className="flex flex-col gap-1">
            ...
            <Badge
              variant="secondary"
              className="w-fit text-[10px] bg-blue-900/50 text-blue-200 border-blue-800"
            >
              Áp dụng: {targetLabel}
            </Badge>
          </div>
          <Badge
            ...
            className={cn(
              ...
              getBadgeVariant(promotion.TrangThai)
            )}
          >
      ...
      <CardContent className="flex-1 space-y-3">
        <div className="bg-primary/10 border-2 border-dashed border-primary/50 rounded-lg p-4 text-center">
          ...
          <p className="text-sm font-semibold text-slate-300 mt-1">
            Mã:
            <span className="ml-1.5 bg-slate-700 text-slate-100 px-2 py-0.5 rounded-md font-mono">
              {promotion.Code}
            </span>
          </p>
        </div>
        ...
        <div className="space-y-1">
          <p className="text-sm text-slate-300">
            Hiệu lực: {format(new Date(promotion.NgayBatDau), "dd/MM/yy")} -{" "}
            {format(new Date(promotion.NgayKetThuc), "dd/MM/yy")}
          </p>
          ...
          <AlertDialogContent className="bg-[#1C1C1C] border-slate-800 text-white">
            <AlertDialogHeader>
              ...
              <AlertDialogDescription className="text-slate-400">
                ...
                {promotion.TenKhuyenMai}&quot; sẽ bị xóa vĩnh viễn (hoặc ẩn đi).
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
...
function PromotionFormDialog({
  ...
  useEffect(() => {
    if (promotion) {
      setFormData({
        ...
        SoLuongMa:
          Number(promotion.SoLuongMa) > 1000000
            ? ""
            : Number(promotion.SoLuongMa),
  ...
  const handleSelectChange = (
    name: "LoaiGiamGia" | "TrangThai" | "DoiTuongApDung",
    value: string
  ) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (
      formData.SoLuongMa !== "" &&
      !positiveIntegerRegex.test(String(formData.SoLuongMa))
    ) {
    ...
    if (
      formData.GiaTriDonToiThieu !== "" &&
      Number(formData.GiaTriDonToiThieu) < 0
    ) {
    ...
    const finalSoLuongMa =
      formData.SoLuongMa === "" || formData.SoLuongMa === 0
        ? 999999999
            ...
            <div className="space-y-4 py-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    value={formData.Code}
                    onChange={handleChange}
                    className="bg-transparent border-slate-700 font-mono uppercase"
                    required
                  />
                </div>
              </div>
              ...
              <div className="space-y-2">
                ...
                <Select
                  ...
                  onValueChange={(v: DoiTuongApDung) =>
                    handleSelectChange("DoiTuongApDung", v)
                  }
                >
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="space-y-2">
                  ...
                  <Select
                    ...
                    onValueChange={(v: LoaiGiamGia) =>
                      handleSelectChange("LoaiGiamGia", v)
                    }
                  >
                    <SelectTrigger className="w-full bg-transparent border-slate-700">
                      <SelectValue />
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="space-y-2">
                  <Label htmlFor="SoLuongMa">
                    Số lượng mã (Bỏ trống = Không giới hạn)
                  </Label>
                  <Input
                    id="SoLuongMa"
                    name="SoLuongMa"
                ...
                <div className="space-y-2">
                  ...
                  <Popover>
                    <PopoverTrigger asChild>
                      <Button
                        ...
                        className={cn(
                          ...
                          !formData.NgayBatDau && "text-slate-400"
                        )}
                      >
                        <CalendarIcon className="mr-2 h-4 w-4" />
                        {formData.NgayBatDau ? (
                          format(new Date(formData.NgayBatDau), "PPP", {
                            locale: vi,
                          })
                        ) : (
                          <span>Chọn ngày</span>
                    ...
                    <PopoverContent className="w-auto p-0 bg-[#1C1C1C] border-slate-700 text-white">
                      <Calendar
                        mode="single"
                        selected={new Date(formData.NgayBatDau)}
                        onSelect={(date) =>
                          handleDateChange("NgayBatDau", date)
                        }
                        initialFocus
                      />
                    </PopoverContent>
                  ...
                  <Popover>
                    <PopoverTrigger asChild>
                      <Button
                        ...
                        className={cn(
                          "w-full justify-start text-left font-normal bg-transparent border-slate-700 hover:bg-slate-800 hover:text-white",
                          !formData.NgayKetThuc && "text-slate-400"
                        )}
                      >
                        <CalendarIcon className="mr-2 h-4 w-4" />
                        {formData.NgayKetThuc ? (
                          format(new Date(formData.NgayKetThuc), "PPP", {
                            locale: vi,
                          })
                        ) : (
                          <span>Chọn ngày</span>
                    ...
                    <PopoverContent className="w-auto p-0 bg-[#1C1C1C] border-slate-700 text-white">
                      <Calendar
                        mode="single"
                        selected={new Date(formData.NgayKetThuc)}
                        onSelect={(date) =>
                          handleDateChange("NgayKetThuc", date)
                        }
                        initialFocus
                      />
                    </PopoverContent>
              ...
              <div className="space-y-2">
                ...
                <Select
                  ...
                  onValueChange={(v) => handleSelectChange("TrangThai", v)}
                >
                  <SelectTrigger className="w-full bg-transparent border-slate-700">
                    <SelectValue />
                  </SelectTrigger>
