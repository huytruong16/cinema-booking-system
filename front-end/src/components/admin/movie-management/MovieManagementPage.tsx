  ...
  return (
    <Card className="bg-[#1C1C1C] border-slate-800 overflow-hidden flex flex-col h-full group hover:border-primary/50 transition-colors">
      ...
      <CardContent className="p-4 flex-1 flex flex-col gap-2">
        <h3
          className="font-bold text-slate-100 line-clamp-1"
        ...
        <div className="flex flex-wrap items-center gap-2 text-xs text-slate-500 mt-1">
          <span className="bg-slate-800 px-1.5 py-0.5 rounded whitespace-nowrap">
            {film.ThoiLuong} phút
          </span>
          <span>•</span>
          <span className="truncate max-w-[120px]">
            {film.NhanPhim?.TenNhanPhim || "Chưa có nhãn"}
          </span>
        </div>
        ...
        <div className="flex items-center gap-2 text-xs text-slate-500">
          ...
          <span className="truncate">
            {film.NgayBatDauChieu
              ? format(new Date(film.NgayBatDauChieu), "dd/MM/yyyy")
              : "N/A"}
          </span>
          <span> - </span>
          <span className="truncate">
            {film.NgayKetThucChieu
              ? format(new Date(film.NgayKetThucChieu), "dd/MM/yyyy")
              : "N/A"}
          </span>
        </div>
...
function FilmFormDialog({
  ...
  useEffect(() => {
    if (film) {
      const rawGenres =
        film.PhimTheLoais?.map(
          (pt: any) => pt.TheLoai?.MaTheLoai || pt.MaTheLoai
        ) || [];
      ...
      setFormData({
        ...
        NgayBatDauChieu: film.NgayBatDauChieu
          ? new Date(film.NgayBatDauChieu)
          : undefined,
        NgayKetThucChieu: film.NgayKetThucChieu
          ? new Date(film.NgayKetThucChieu)
          : undefined,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (
        !formData.TenHienThi ||
        !formData.TenGoc ||
        !formData.NgayBatDauChieu ||
        !formData.NgayKetThucChieu ||
        !formData.MaNhanPhim
      ) {
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="lg:col-span-2 space-y-4">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  ...
                  <div className="col-span-2 space-y-2">
                    ...
                    <Select
                      ...
                      onValueChange={(val) =>
                        setFormData((prev) => ({ ...prev, MaNhanPhim: val }))
                      }
                    >
                      ...
                      <SelectContent className="bg-[#1C1C1C] text-slate-100 border-slate-700">
                        {labelsList.map((l: any) => (
                          <SelectItem key={l.MaNhanPhim} value={l.MaNhanPhim}>
                            {l.TenNhanPhim}
                          </SelectItem>
                        ))}
                      </SelectContent>
                    ...
                    <Popover>
                      <PopoverTrigger asChild>
                        <Button
                          variant={"outline"}
                          className={cn(
                            "w-full justify-start text-left font-normal bg-transparent border-slate-700",
                            !formData.NgayBatDauChieu && "text-muted-foreground"
                          )}
                        >
                          <CalendarIcon className="mr-2 h-4 w-4" />
                          {formData.NgayBatDauChieu ? (
                            format(formData.NgayBatDauChieu, "PPP", {
                              locale: vi,
                            })
                          ) : (
                            <span>Chọn ngày</span>
                      ...
                      <PopoverContent
                        className="w-auto p-0 bg-[#1C1C1C] border-slate-700 text-white"
                        align="start"
                      >
                        <Calendar
                          mode="single"
                          selected={formData.NgayBatDauChieu}
                          onSelect={(date) =>
                            setFormData((prev) => ({
                              ...prev,
                    ...
                    <Popover>
                      <PopoverTrigger asChild>
                        ...
                        >
                          <CalendarIcon className="mr-2 h-4 w-4" />
                          {formData.NgayKetThucChieu ? (
                            format(formData.NgayKetThucChieu, "PPP", {
                              locale: vi,
                            })
                          ) : (
                            <span>Chọn ngày</span>
                      ...
                      >
                        <Calendar
                          mode="single"
                          selected={formData.NgayKetThucChieu}
                          onSelect={(date) =>
                            setFormData((prev) => ({
                              ...prev,
                  ...
                  <Popover>
                    <PopoverTrigger asChild>
                      <Button
                        variant="outline"
                        role="combobox"
                        className="w-full justify-between bg-transparent border-slate-700 h-auto min-h-[40px] py-2"
                      >
                          ...
                          <CommandGroup>
                            {genresList.map((genre: any) => (
                              <CommandItem
                                key={genre.MaTheLoai}
                                onSelect={() =>
                                  handleGenreToggle(genre.MaTheLoai)
                                }
                                className="cursor-pointer hover:bg-slate-800"
                              >
                                <div className="flex items-center gap-2">
                                  <Checkbox
                                    checked={formData.TheLoais.includes(
                                      genre.MaTheLoai
                                    )}
                                    onCheckedChange={() =>
                                      handleGenreToggle(genre.MaTheLoai)
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    name="DanhSachDienVien"
                    value={formData.DanhSachDienVien}
                    onChange={handleChange}
                    className="bg-transparent border-slate-700"
                    placeholder="A, B, C..."
                  />
                ...
                <div className="space-y-2">
                  ...
                  <Textarea
                    ...
                    onChange={handleChange}
                    className="bg-transparent border-slate-700 min-h-[100px]"
                  />
                </div>
              </div>
            </div>
        ...
        <DialogFooter className="p-4 sm:p-6 pt-2 border-t border-slate-800 bg-[#1C1C1C] shrink-0 flex-col sm:flex-row gap-2">
          <DialogClose asChild>
            <Button
              variant="outline"
              type="button"
              className="bg-transparent border-slate-700 w-full sm:w-auto mt-2 sm:mt-0"
